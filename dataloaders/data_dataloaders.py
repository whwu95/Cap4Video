...
#     ori_data = list(zip(*data))
#     caption, video, video_mask = ori_data[0], ori_data[1], ori_data[2]
#     video = torch.Tensor(video)
#     video_mask = torch.Tensor(video_mask)
#     return caption, video, video_mask
from dataloaders.dataloader_vatex_retrieval import VATEX_DataLoader

def collate_function(data):
    ori_data = list(zip(*data))
    pairs_text, pairs_mask, pairs_segment, video, video_mask, captions = ori_data[0], ori_data[1], ori_data[2], ori_data[3], ori_data[4], ori_data[5]
    video = torch.Tensor(video)
    video_mask = torch.LongTensor(video_mask)
    pairs_text = torch.LongTensor(pairs_text)
    pairs_mask = torch.LongTensor(pairs_mask)
    pairs_segment = torch.LongTensor(pairs_segment)
    return pairs_text, pairs_mask, pairs_segment, video, video_mask, captions


def dataloader_msrvtt_train(args, tokenizer, load_video, caption_aug, generate_text_backbone_clip_bert=0,original_text_backbone_clip_bert=0):
...
def dataloader_msrvtt_test(args, tokenizer,load_video, subset="test", generate_text_backbone_clip_bert=0,original_text_backbone_clip_bert=0):
...
def dataloader_didemo_test(args, tokenizer, subset="test"):
    ...
    dataloader_didemo = DataLoader(
        ...
        shuffle=False,
        drop_last=False,
        # collate_fn=collate_function ## new
    )
